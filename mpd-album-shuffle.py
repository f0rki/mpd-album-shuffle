    ...
    def __enter__(self):
        ...
        if self.password is not None:
            try:
                ...
                return None
        logging.debug("connected to MPD version: {}".format(self.client.mpd_version))
        return self.client
...
class AlbumList(object):
    ...
    def choose_random_album(self, current):
        """picks a random album from the current playlist, while trying to avoid
        the current album.
        """
        if self.albums:
    ...
    def shuffle_albums(self):
        """Essentially do a random.shuffle over all albums and sort the playlist
        accordingly. This assumes that the playlist is already sorted by albums.
        """
...
def main():
    parser = argparse.ArgumentParser(\
            description="Randomly play or sort the albums in the current mpd playlist")
    ...
    if args.host:
        host = args.host[0]
    logging.debug("connecting to host: {} port: {} pw: {}".format(host, port, pw))
